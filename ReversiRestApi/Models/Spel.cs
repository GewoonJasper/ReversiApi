...
namespace ReversieISpelImplementatie.Model
{
    public class Spel : ISpel
    {
        ...
        public string BordInString { get; set; }
        ...
        public Spel()
        {
            ...
            Beurt = 0;
            AantalFichesWitPerBeurt = "2";
            AantalFichesZwartPerBeurt = "2";

            ConvertBordToString(Bord);

            AandeBeurt = Kleur.Geen;
            Status = Status.Wachten;

            if (!string.IsNullOrEmpty(Speler2Token)) {
        ...
        public void Opgeven()
        {
            Winnaar = GetKleurTegenstander(AandeBeurt);
        ...
        public bool IsErEenZetMogelijk(Kleur kleur)
        {
            ...
            return false;
        } // Deze was private maar heb ik public gemaakt
        ...
        public Spel DoeZet(int rijZet, int kolomZet)
        {
            if (ZetMogelijk(rijZet, kolomZet))  // todo: maak hierbij gebruik van de reeds in deze klassen opgenomen methoden!
            {
                for (int i = 0; i < 8; i++)
                {
        ...
        public void ConvertBordToString(Kleur[,] bord)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    bordString.Append($"{bord[i, j]}-");
                }
            }

            BordInString = bordString.ToString();
        }

        public void ConvertStringToBord()
        {
            string[] bordSplit = BordInString.Split("-");
            Kleur[,] kleur = new Kleur[8, 8];
            int x = 0;
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    Kleur kleurX = Enum.Parse<Kleur>(bordSplit[x]);
